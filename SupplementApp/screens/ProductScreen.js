  ...
  return (
    <ScrollView contentContainerStyle={{ padding: spacing.lg }} nestedScrollEnabled>
      ...
      {/* Essentials */}
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={styles.essentialItem} onPress={() => { if (fromHome) { navigation.push("EssentialScannerFromProductScanner", { essentialName: item.name }) } else { navigation.navigate("Essential", { essentialName: item.name, essentialHumanName: item.human_name }) } }}>
        ...
        <>
          <FlatList
            ...
            renderItem={({ item }) => (
              ...
              >
                <ProductImageById
                  loading={loadingProduct && !notFound}
                  productId={item.id}
