...
# Remove system setuptools/pkg_resources to avoid symbol bug
...
# Final setuptools safeguard
RUN rm -rf /opt/venv/lib/python3.10/site-packages/pkg_resources* && \
    pip install --upgrade --force-reinstall setuptools==70.0.0

# Default command: start Flask + SocketIO with Gunicorn
CMD ["gunicorn", "-b", "0.0.0.0:5000", "backend_server.app:app", "--worker-class", "gevent", "--workers", "1"]
